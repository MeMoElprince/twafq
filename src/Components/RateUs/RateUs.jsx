...
export default function RateUs() {
  ...
  function handleSubmit(event) {
    ...
    if (!currFormData.message) return;
    setButtonActive(false);
    Swal.fire({
      title: isRTL ? "نشر قصتك" : "Share Your Story",
      text: isRTL
        ? "هل أنت متأكد أنك تريد نشر قصتك؟"
        : "Are you sure you want to share your story?",
      icon: "warning",
      ...
      scrollbarPadding: false,
        ...
        if (result.isConfirmed && Token && currFormData.message !== "") {
          event.preventDefault();
          Fetch({
            url: `${putReview()}userId=${formData.id}&story=${
              currFormData.message
            }`,
            Token,
            method: "POST",
          });
  ...
  return (
    <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit}>
      <label
        htmlFor="myMessage"
        className="myFont text-2xl md:text-3xl w-max sm:text-3xl font-medium ml-1 text-White"
      >
      ...
      <textarea
        ...
        rows="6"
        name="message"
        value={currFormData.message}
        onChange={handleChange}
        placeholder={isRTL ? "اكتب قصتك..." : "Type your story..."}
        className="w-full rounded-md px-4 bg-[#E1E1E1] text-Black text-sm py-3 outline-Black resize-none"
      ></textarea>
      <button
        className={`text-white ${
          !buttonActive && "pointer-events-none opacity-50"
        } bg-[#e42041] hover:bg-[#c12640] tracking-wide rounded-md text-lg px-4 py-3 w-full !mt-6`}
      >
        {isRTL ? "مشاركة" : "Share"}
      </button>
    </form>
