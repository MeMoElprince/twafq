...
const Navbar = ({setPopType, setPopActive}) => {
  ...
  function handleLogout(){
    Swal.fire({
      ...
      text: isRTL ? "هل تريد تأكيد تسجيل الخروج؟" : "Are you sue you want to logout?",
      icon: "warning",
  ...
  return (
    <nav className="fixed top-0 left-0 bg-LighterPink shadow-lg myFont w-full border-b-2 border-black/5 z-50" aria-label={"navigation"}>
      <div className="w-[98%] sm:px-6 lg:px-8 flex items-center justify-between h-20 center">
        <div className="center w-[90%]">
          ...
          <div className={`hidden md4:flex items-baseline gap-4 lg:gap-8 ${isRTL ? "mr-auto" : "ml-auto"}`}>
            ...
            <Link to="/explore/0" onClick={handleChange} aria-label={t("navbar.partner")} className="text-Black focus:outline-none transition-colors duration-150 hover:text-DarkPink rounded-md text-base font-medium">{t("navbar.partner")}</Link>
        ...
        <button onClick={() => setIsRTL(!isRTL)} aria-label={isRTL ? "التحويل إلى الانجليزية" : "Switch to Arabic"} className={`absolute ${isRTL ? "md4:left-[2%] left-[7%]" : "md4:right-[2%] right-[7%]"} inline-flex items-center justify-center rounded-md text-Black hover:text-white focus:outline-none`}>
          <p className='myFont text-lg bg-gray-700 text-White px-4 py-1.5 rounded-full'>{isRTL ? "EN" : "عر"}</p>
        </button>
        <button onClick={toggleMenu} aria-label={isOpen ? isRTL ? "غلق القائمة" : "Close Menu" : isRTL ? "فتح القائمة" : "Open Menu"} className={`md4:hidden absolute ${isRTL ? "right-[7%]" : "left-[7%]"} inline-flex items-center justify-center rounded-md text-Black focus:outline-none`}>
      ...
      {isOpen && (
        <div className="md4:hidden mt-6 pt-2 pb-6 flex flex-col gap-4 w-full justify-center items-center sm:px-3">
          ...
          <Link to="/explore/0" onClick={handleChange} aria-label={t("navbar.partner")} className="text-Black hover:text-DarkPink transition-colors duration-150 block rounded-md text-base font-medium">{t("navbar.partner")}</Link>
