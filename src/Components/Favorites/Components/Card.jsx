...
export default function Card({ id, formData, isLogedIn, Token }) {
  ...
  return (
    <div className="group flex relative flex-col items-center gap-10 rounded-md px-2.5 py-3 transition-all duration-75 hover:bg-gray-200/80">
      ...
      <Link to={`userProfile/${id}`} aria-label="Link to profile" className="flex items-center gap-x-5 w-full">
      ...
      <div className="center gap-3 max-w-sm w-full overflow-visible">
          <a href={`tel:${dataA?.phone}`} className="py-2.5 w-[200px] rounded-full text-sm font-medium text-DarkPink  border-2 border-DarkPink backdrop-blur-lg hover:bg-DarkPink/5 shadow-lg hover:w-[220px] transition-all duration-300">
            <div className="center gap-4">
              <p className="myFont tracking-wider">{dataA?.phone}</p>
              <MdContactPhone
