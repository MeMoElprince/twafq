export function handleLoginInfo(formData, setErrorMessage) {
    ...
    if (!formData.gender || formData.gender[0] === "" || formData.gender[1] === "") {
    ...
    if (!formData.email || !emailRegex.test(formData.email)) {
      errorMessage.push(["يرجى إدخال البريد الإلكتروني بشكل صحيح", "Please enter a valid email address"]);
    }
    ...
    if (formData.confirmPassword !== formData.password && !test) {
      errorMessage.push(["كلمة المرور غير متطابقة", 
...
export function handlePersonalInfo(formData, setErrorMessage) {
    ...
    if (!namePattern.test(username)) {
        errorMessage.push(["يجب أن يكون الاسم بين 3 و 30 حرفًا ويتكون فقط من الحروف والأرقام", "Username must be between 3 and 30 characters long and can only contain letters and numbers."]);
    }
...
export function handleFamilyStatus(formData, setErrorMessage) {
    ...
    labels.map((label) => {
        if((label === 'children' && formData.label) || (label !== 'children' && (!formData[label][1] || !formData[label][0]))){
            if(!testFound)
                errorMessage.push(["الرجاء ملء جميع الحقول", "Please fill all the required"]);
            testFound = 1;
        }
