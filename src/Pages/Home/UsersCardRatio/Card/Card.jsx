...
export default function Card({ userInfo, isLogedIn }) {
    ...
    <div className="">
      {!loadingUser && (
        <div className="bg-white p-6 select-none relative pt-[82px] max-h-[500px] min-h-[500px] border border-Black/20 w-[285px] md:w-[320px] lg2:w-[290px] rounded-2xl myFont overflow-hidden mx-auto lg2:mx-0">
            ...
            {isLogedIn ? (
              <>
                <div className={`center w-full gap-2 z-20`}>
                  ...
                  <p className="text-lg font-medium text-Black">
                    {Math.round(userInfo?.presentage)}&#x25;
                  </p>
                </div>
                <div
                  ...
                  style={{ width: `${Math.round(userInfo?.presentage)}%` }}
                ></div>
              </>
            ) : (
          ...
          <Link to={`userprofile/${userInfo.userId}`} aria-label={isRTL ? `الملف الشخصي الخاص بالمستخدم ${userDetails?.firstName}` : `User ${userDetails?.firstName} profile`} className="flex flex-col items-center gap-4">
